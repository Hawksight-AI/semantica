...
- **Running range checks under RDFS inference**: `rdfs:range` is an *entailment* rule, not a constraint — with `inference="rdfs"` every object of the property is inferred into the range class, so an `sh:class` range check on it always passes. Validate those shapes with `inference="none"`, or omit `rdfs:range` for properties you want SHACL to police. The trade-off is that `sh:targetClass` stops reaching subclasses. Semantica's `_run_pyshacl` helper always runs with `inference="none"`, so this pitfall only applies when you call pySHACL directly with inference enabled. Turning inference *on* reads like making validation stricter, but for range checks it does the opposite.
