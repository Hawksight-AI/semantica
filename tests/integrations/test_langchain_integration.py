...
class TestToolsWithLangchain(unittest.TestCase):
    ...
    def test_kg_tool_builds_structured_tool(self):
        ...
        graph = ContextGraph()
        graph.add_node(
            node_id="alice", node_type="person", content="Alice is a developer"
        )
...
class TestRetrieverBehavior(unittest.TestCase):
    ...
    def test_hit_id_unwraps_context_graph_query_shape(self):
        from integrations.langchain.retriever import _hit_id
        ...
        self.assertEqual(_hit_id(hit), "alice")
        self.assertEqual(_hit_id({"node_id": "n1"}), "n1")
        self.assertEqual(_hit_id({"id": "n2"}), "n2")
