...
- **Provenance timestamps used deprecated `datetime.utcnow()`, and KG workflow tests asserted IDs without reading stored records** (#946, closes #946) by @pravit-amp
  ...
  - `query_recorded_between()` and `audit_log()` parse timestamps before comparing them instead of comparing ISO strings directly, so a store holding both pre-existing naive stamps and the new offset-bearing ones filters and sorts on the instant. Comparing the two forms as strings put an offset-bearing record above a naive bound at the identical instant, excluding it from a range that should contain it; naive values are read as UTC, matching `ProvenanceTracker._parse_dt()`
