...
- **Salesforce ingestor** (`SalesforceIngestor`, closes #1227)
  - New `pip install semantica[db-salesforce]` extra (`simple-salesforce>=1.12.0`), included in `db-all`
  - `SalesforceConnector` manages the `simple-salesforce` client lifecycle with username/password/security-token authentication (standard SOAP login), session-ID + instance-URL authentication (pre-existing OAuth sessions), sandbox (`domain="test"`) and production (`domain="login"`) support, configurable API version, and full env-var fallback (`SALESFORCE_USERNAME`, `SALESFORCE_PASSWORD`, `SALESFORCE_SECURITY_TOKEN`, `SALESFORCE_DOMAIN`, `SALESFORCE_INSTANCE_URL`, `SALESFORCE_SESSION_ID`, `SALESFORCE_API_VERSION`)
  - `SalesforceIngestor` provides `ingest_sobject()` (SOQL construction with sObject name, field name, WHERE, and ORDER BY injection-safety validation; automatic `nextRecordsUrl` pagination; compound field exclusion), `ingest_query()` (raw SOQL passthrough with pagination), `list_sobjects()`, `get_sobject_schema()`, `export_as_documents()` (matches Snowflake/Databricks document format), and context-manager support
  - `SalesforceData` dataclass carries `data`, `row_count` (records actually returned), `columns`, `sobject`, `query`, `instance_url`, `total_size` (Salesforce `totalSize` before any limit), `metadata`, and `ingested_at`
  - `ingest_salesforce()` convenience function with methods `"sobject"`, `"query"`, `"list_sobjects"`, `"schema"`, and `"documents"` (ingest + export in one call); registered in `MethodRegistry` and wired into the unified `ingest(source_type="salesforce", ...)` dispatcher
  - SOQL injection hardening: sObject names validated against `^[A-Za-z][A-Za-z0-9_]*(__c|__mdt|__e|…)?$`; field names validated per component (dot-notation allowed); WHERE fragments checked against a keyword/separator blocklist with SOQL-correct `''`-style literal masking; ORDER BY validated to column names with optional ASC/DESC and NULLS FIRST/LAST; `limit=0` short-circuits before any network call; `LIMIT 0` never emitted in SOQL
  - `docs/integrations/salesforce.md` integration guide; `docs/docs.json` navigation entry
  - 184 unit tests (all mocked — no Salesforce account required)
  - **Not yet implemented**: JWT Bearer (`consumer_key` + private key) and Bulk API 2.0
