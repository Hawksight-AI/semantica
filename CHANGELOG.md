...
- **`AgnoDecisionKit.check_policy` silently treated unevaluable policy rules as compliant** (#778) by @Sameer6305
  ...
  - Added 10 tests to `tests/integrations/agno/test_decision_kit.py` covering the missing-field case (the issue's traced example), the malformed-rule-string case, the bare-JSON-string `policy_rules` amplification case, non-list/non-string `policy_rules` shapes, the missing-key-vs-null-value distinction, and regression checks confirming normal rule evaluation on present fields is unchanged
